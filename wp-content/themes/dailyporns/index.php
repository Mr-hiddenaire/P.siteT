  ...
  <div id="primary" class="columns">
        ...
        if ( have_posts() ) : ?>
          <div class="column">
            ...
            <?php if (is_front_page() && is_home() && !(is_paged()) ) : echo esc_html(richflicks_themeoptions('frontpageheadline')); 
                  elseif ( is_category() ) : printf( single_cat_title( '', false )  );
                  elseif ( is_tag() ) : printf( single_tag_title( '', false )  );
                  elseif ( is_author() ) : echo get_the_author(); 
                  elseif ( is_search() ) : printf( __( 'Search for: %s', 'richflicks' ), '<i>' . get_search_query() . '</i>' ); 
                  elseif ( is_day() ) : printf( get_the_date() ); 
                  elseif ( is_month() ) : printf( get_the_date('F Y') ); 
                  elseif ( is_year() ) :  printf( get_the_date('Y') ); 
                  endif; 
            ?>
            </h5>
          </div>
